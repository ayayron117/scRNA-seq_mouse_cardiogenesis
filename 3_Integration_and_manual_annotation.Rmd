---
title: "Integration & Manual Annotation"
...
```{r Integration and UMAP function}
...
```{r Identification of blood clusters}
...
```{r Removal of scDblfinder doublets}
...
```{r Get markers of small outlier cluster}
...
```{r Annotation of broad cell types}
...
```{r Integration without CC regression}
...
```{r Reintroduce scDblfinder doulets}
...
```{r Identify cells that clustered with scDblfinder doulets}
...
```{r Remove cells that clustered with scDblfinder doulets}
...
```{r Annotation of myocardial cells}
...
```{r Remove the same cells from the main seurat}
