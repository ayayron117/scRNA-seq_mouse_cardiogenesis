...
seurat_input_dir <- file.path(dirname(getwd()),
                              "3_Integration_and_manual_annotation",
                              "RDS_files")
...
``` r
myo <- readRDS(file.path(seurat_input_dir,
                         "9_final_myocardial_seurat.rds"))
```

``` r
DefaultAssay(myo) <- "RNA"
Idents(myo) <- "myocardial_celltypes"

atrial <- subset(myo, idents = "Atrial")
atrial <- ScaleData(atrial, row.names(atrial))

ventriclular <- subset(myo, idents = "Ventricular")
...
dge.analysis <- function (seurat, cluster, methods, ident.1, ident.2,
  ...
  for (m in methods) {
    ...
    union_df <- rbind(up_union_list[[m]], down_union_list[[m]])
    union_df <- union_df[order(-union_df$avg_log2FC), ]
    
    try(go.gsea(df = union_df, 
                up_dir = union_up, 
                down_dir = union_down, 
                fileName = paste0(m, sep = "_",
                                  "union", sep="_",
                                  cluster), 
                pCutoff = pCutoff,
                db= "org.Mm.eg.db",
    ...
    inter_df <- rbind(up_intersection_list[[m]], down_intersection_list[[m]])
    inter_df <- inter_df[order(-inter_df$avg_log2FC), ]
    
    try(go.gsea(df = inter_df, 
                up_dir = inter_up, 
                down_dir = inter_down, 
                fileName = paste0(m, sep = "_",
                                  "intersection", sep="_",
...
``` r
padj_dir <- file.path(dge_dir, "colored_by_padjusted")
dir.create(padj_dir)

dge.analysis(seurat = atrial, cluster = "Atrial", methods = c("DESeq2","MAST","wilcox"),
             ident.1 = "KO", ident.2 = "WT",
             FCcutoff = 0.1, pCutoff = 0.05,
             path= padj_dir,
             color_by = "p.adjust")

dge.analysis(seurat = ventriclular, cluster = "Ventriclular", methods = c("DESeq2","MAST","wilcox"),
             ident.1 = "KO", ident.2 = "WT",
             FCcutoff = 0.1, pCutoff = 0.05,
             path= padj_dir,
             color_by = "p.adjust")

dge.analysis(seurat = SAN, cluster = "SAN", methods = c("DESeq2","MAST","wilcox"),
             ident.1 = "KO", ident.2 = "WT",
             FCcutoff = 0.1, pCutoff = 0.05,
             path= padj_dir,
             color_by = "p.adjust")

dge.analysis(seurat = OFT, cluster = "OFT", methods = c("DESeq2","MAST","wilcox"),
             ident.1 = "KO", ident.2 = "WT",
             FCcutoff = 0.1, pCutoff = 0.05,
             path= padj_dir,
             color_by = "p.adjust")

dge.analysis(seurat = AVC, cluster = "AVC", methods = c("DESeq2","MAST","wilcox"),
             ident.1 = "KO", ident.2 = "WT",
             FCcutoff = 0.1, pCutoff = 0.05,
             path= padj_dir,
             color_by = "p.adjust")
...
``` r
pval_dir <- file.path(dge_dir, "colored_by_pvalue")
dir.create(pval_dir)
...
writeLines(capture.output(sessionInfo()),  
           file.path(getwd(), "4_sessionInfo.txt"))
```
